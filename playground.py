...
class FaceMeshDetector:
    ...
    def find_face_mesh(self, img):
        eyes_landmarks = {"left_eye": [], "right_eye": [], "left_iris": [], "right_iris": [],
            ...
            for faceLms in results.multi_face_landmarks:
                for i, lm in enumerate(faceLms.landmark):
                    ...
                    if i in self.LEFT_EYE_LANDMARKS:
                        eyes_landmarks["left_eye"].append((x, y))
                    if i in self.RIGHT_EYE_LANDMARKS:
                        eyes_landmarks["right_eye"].append((x, y))
                    if i in self.LEFT_IRIS_LANDMARKS:
                        eyes_landmarks["left_iris"].append((x, y))
                    if i in self.RIGHT_IRIS_LANDMARKS:
                        eyes_landmarks["right_iris"].append((x, y))

        # with self.camera.landmarks_lock:
        eyes_landmarks["l_iris_center"] = iris_center(eyes_landmarks["left_iris"])
        eyes_landmarks["r_iris_center"] = iris_center(eyes_landmarks["right_iris"])

        return img, eyes_landmarks
...
if __name__ == '__main__':
    ...
    while True:
        ...
        image, landmarks = detector.find_face_mesh(image)
